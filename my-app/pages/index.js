...
export default function Home() {
  ...
  // tokensMinted is the total number of tokens that have been minted till now out of 1000(max total supply)
  ...
  const getBalanceOfOnigiriTokens = async () => {
    try {
      ...
      setBalanceOfOnigiriTokens(balance);
    } catch (err) {
      console.error(err);
      setBalanceOfOnigiriTokens(zero);
    }
  };

  /**
   * mintOnigiriToken: mints `amount` number of tokens to a given address
   */
  const mintOnigiriToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Successfully minted Onigiri Tokens");
      await getBalanceOfOnigiriTokens();
  ...
  const claimOnigiriTokens = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully claimed Onigiri Tokens");
      await getBalanceOfOnigiriTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    // If user is not connected to the Mantle Testnet network, let them know and throw an error
    const { chainId } = await web3Provider.getNetwork();
    if (chainId !== 5001) {
      window.alert("Change the network to Mantle Testnet");
      throw new Error("Change network to Mantle Testnet");
    }
    ...
    if (tokensToBeClaimed > 0) {
      return (
        <div>
          ...
          <button className={styles.button} onClick={claimOnigiriTokens}>
            Claim Tokens
          </button>
        </div>
      );
    ...
    return (
      <div style={{ display: "flex-col" }}>
        ...
        <button
          ...
          onClick={() => mintOnigiriToken(tokenAmount)}
        >
          Mint Tokens
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Onigiri ICO!</h1>
          <div className={styles.description}>
            You can claim or mint Onigiri tokens here
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfOnigiriTokens)}{" "}
                Onigiri Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
      ...
      <footer className={styles.footer}>Made with &#10084; by Noobsters</footer>
    </div>
  );
}
